...
public class BD_Colegio extends BD_Conector {
	...
	public int añadir_Curso(Curso cur) throws ErrorBaseDatos {
		String cadenaSQL = "INSERT INTO cursos VALUES('" + cur.getCurso() + "','" + cur.getDescripcion() + "','"
				+ cur.getAula() + "')";
	...
	public Alumno buscar_Alumno(String dni) throws ErrorBaseDatos {
		...
		try {
			...
			if (reg.next()) {
				...
				// listaCursos.add(new
				// Alumno(reg.getString("dni"),reg.getString("nombre"),reg.getString("curso"),reg.getInt("matricula"),reg.getString("telefono"),fBuena));
				al = new Alumno(reg.getString("dni"), reg.getString("nombre"), reg.getString("curso"),
						reg.getInt("matricula"), reg.getString("telefono"),
						reg.getDate("fechaMatricula").toLocalDate());
	...
	public ArrayList<Alumno> buscar_Alumno_Mat(int mat) throws ErrorBaseDatos {
		...
		try {
			...
			while (reg.next()) {
				...
				// listaCursos.add(new
				// Alumno(reg.getString("dni"),reg.getString("nombre"),reg.getString("curso"),reg.getInt("matricula"),reg.getString("telefono"),fBuena));
				alumnos.add(new Alumno(reg.getString("dni"), reg.getString("nombre"), reg.getString("curso"),
						reg.getInt("matricula"), reg.getString("telefono"),
						reg.getDate("fechaMatricula").toLocalDate()));
