...
// TODO Auto-generated method stub
		...
		System.out.println("beneficio total:" + Almacen.getBeneficio());
		int cont = 0;
		do {
			...
			case 4:
				...
				for (int i = 0; i < cont; i++) {
					if (Almacen.get(i).getNombre().equalsIgnoreCase(nombre)) {
						double importe = Almacen.get(i).vender(kilos);
						if (importe != 0) {
							break;
						}
					}
	...
	public static void crearCargamento(Scanner sc, int val, ArrayList<Almacen> Almacen) {
		String nombre;
		String procedencia;
		double kilos;
		double coste;
		...
		Almacen.add(new Almacen(nombre, procedencia, kilos, coste, venta));
