...
public class Representacion {
	...
	public Representacion(String titulo, LocalDate fecha) {
		...
		this.codigo = (titulo.substring(0, 3)).toUpperCase() + "-" + fecha.getMonthValue() + fecha.getDayOfMonth();
		...
		this.codigo =titulo.substring(0, 3).toUpperCase()+"_"+mesf+diaf;
	}
	...
	/*public void ocuparAsientos (int fila, int columna) {
		boolean esVip=false, ocupado=false;
		do {
			boolean filaCorrecta=(fila>=0 && fila<8 && columna>=0 && columna <=5 );
			if(fila<3 && fila>=0 ) {
				esVip=true;
			}
			if(fila>8 || columna >5 || columna <0) {
				System.out.println("no ha sido posible asignarb los asientos, vuelva a intentarlo");
			}
			if(filaCorrecta && asientos[fila][columna]==0) {
				asientos[fila][columna]=1;
				ocupado=true;
				System.out.println("el asiento se ha ocupado con exito");}
			else {
					System.out.println("el asiento ya se encuentra ocupado");}
		}while(!ocupado);
	}*/
	// Devuelve true si pudo ocupar el asiento, false si estaba ocupado o mal la fila
