...
public class main {
	...
	public static void main(String[] args) {
		...
		citaMedica citas[] = new citaMedica[15];
		...
		do {
			...
			case 1:
				if (cont == citas.length) {
					...
					citas[cont] = new citaMedica(nombre, motivo, fecha);
					cont++;
			...
			case 2:
				...
				citas[pos] = new citaMedica(nombre, motivo, fecha);
				}
				break;
		...
		/*	case 4:
			    System.out.println("\n--- LISTADO DE CITAS ---");
			    for(int i = 0; i < cont; i++) {
			        // Usamos el patrón "dd/MM/yyyy HH:mm" que definiste en el main
			        String fechaFormateada = citas[i].getFecha().format(patron);
			        System.out.println("ID: " + citas[i].getCodigo() + 
			                           " | Mascota: " + citas[i].getNombre() + 
	...
	private static int buscarCita(citaMedica[] citas, Scanner sc, int cont) {
		String codigo;
		int pos = -1;
		do {
